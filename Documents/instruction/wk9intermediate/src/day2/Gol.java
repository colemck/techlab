	...
	public static boolean[][] life(boolean[][] cells) {
		...
		for (int r = 0; r < rs; r++) {
			...
			for (int c = 0; c < cs; c++) {
				...
				// top left
				if (r > 0 && c > 0 && cells[r - 1][c - 1] == true) neighbors++;
				
				// above
				if (r > 0 && cells[r - 1][c] == true) neighbors++;
				
				// top right
				if (r > 0 && c < cs - 1 && cells[r - 1][c + 1]) neighbors++;
				
				// right
				if (c < cs - 1 && cells[r][c + 1] == true) neighbors++;
				
				// bottom right
				if (r < rs - 1 && c < cs - 1 && cells[r + 1][c + 1] == true) neighbors++;
				
				// below
				if (r < rs - 1 && cells[r + 1][c] == true) neighbors++;
				
				// bottom left
				if (c > 0 && r < rs - 1 && cells[r + 1][c - 1]) neighbors++;
				
				// left
				if (c > 0 && cells[r][c - 1] == true) neighbors++;
